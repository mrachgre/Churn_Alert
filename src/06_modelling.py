...
Step 6: Modelling — XGBoost + Optuna + CalibratedClassifier + SHAP
         + Logistic Regression (chi2-selected linear features from Step 5b)
...
Data sources per model
  LR-Base   → X_train_linear.csv      (Step 5b: outlier-removed, chi2-selected)
  LR-Dist   → X_train_linear_dist.csv (Step 5b: base + K-Means dist, chi2-selected)
...
def load_feature_variants():
    """Load K-Means cluster-ID splits produced by Step 5 (for Tree models)."""
...
def load_linear_data():
    """
    Load chi2-selected, outlier-cleaned linear data from Step 5b.
    LR-Base  uses base features only.
    LR-Dist  uses base + K-Means distance features.
    Each set has its own y_train (SMOTE done inside Step 5b).
    """
...
def run_modelling():
    ...
    # ── Linear models (Step-5b chi2-selected features) ───────────────────────
    print(f"\n[06] ── Linear Models (chi2-selected, outlier-cleaned) ─────────────")
    (X_tr_lin, X_te_lin, y_tr_lin,
     X_tr_lin_dist, X_te_lin_dist, y_tr_lin_dist,
     y_test_lin) = load_linear_data()
    print(f"[06] LR-Base  features: {X_tr_lin.shape[1]}  "
          f"| train rows: {len(X_tr_lin):,}")
    print(f"[06] LR-Dist  features: {X_tr_lin_dist.shape[1]}  "
          f"| train rows: {len(X_tr_lin_dist):,}")
    ...
    # ── Tree models (Step-5 K-Means cluster-ID features) ─────────────────────
    ...
    # XGBoost retrained with cluster-ID feature
