...
Run this from the project root directory:
    ...
    python run_pipeline.py --dataset E_Commer_Data   (default)
    python run_pipeline.py --dataset E_commer_Data_OLIST
"""
...
# ── Dataset selection ─────────────────────────────────────────────────────────
# Change this value (or pass --dataset <name>) to target a different dataset.
_args    = sys.argv[1:]
DATASET  = _args[_args.index("--dataset") + 1] if "--dataset" in _args else "E_Commer_Data"

# MUST be set BEFORE any step module is imported
os.environ["PIPELINE_DATASET"] = DATASET

# Ensure src/ is on the path so steps can `from pipeline_config import ...`
...
def main():
    ...
    # ── Step 5b ──────────────────────────────────────────────────────────
    banner("Step 5b: Linear Model Preprocessing (Outlier Removal + Chi-Square)")
    load_src("05b_linear_preprocessing.py").run_linear_preprocessing()
    ...
    print(f"  Outputs  → outputs/{DATASET}/")
    print(f"  Models   → models/{DATASET}/")
    print(f"\n  ▶  Launch dashboard:")
