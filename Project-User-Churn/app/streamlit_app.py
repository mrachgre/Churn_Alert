...
def render_research_insights():
    ...
    panels = [
        ("Model Stability", "model_stability.csv",
         "Reports repeated stratified K-fold accuracy, F1, and ROC-AUC stability."),
        ("Feature Redundancy", "feature_redundancy_analysis.csv",
         "Ranks dominant predictors using mutual information, correlation, and SHAP impact."),
        ("Research Findings", "research_findings.csv",
         "Summarizes each research question, method, result, conclusion, and business impact."),
        ("Fairness", "fairness_extended_report.csv",
         "Reports group metrics plus demographic parity and equal opportunity differences."),
        ("Business ROI", "business_roi_analysis.csv",
         "Compares models using expected cost, cost reduction, retained customers, retention value, and ROI score."),
    ]

    for title, filename, note in panels:
        df = load_output_csv(filename)
        with st.expander(title, expanded=(title == "Research Findings")):
            st.info(note)
            if df is None or df.empty:
                st.warning(f"{filename} not found. Run `python run_pipeline.py` to generate it.")
            else:
...
page = st.sidebar.radio("Page", ["Customer Lookup", "Research Insights"], index=0)
