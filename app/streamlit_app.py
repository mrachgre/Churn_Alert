...
# Default dataset — can be overridden by env var or sidebar
_ENV_DATASET = os.environ.get("PIPELINE_DATASET", "E_Commer_Data")
...
with st.sidebar:
    ...
    ]) if os.path.isdir(_data_root) else [_ENV_DATASET]

    _default_idx = _available.index(_ENV_DATASET) if _ENV_DATASET in _available else 0
    DATASET = st.selectbox("Active dataset", _available, index=_default_idx,
                           key="dataset_selector")
    DATA  = os.path.join(_ROOT, "data",   DATASET)
    MODEL = os.path.join(_ROOT, "models", DATASET)
    st.caption(f"`data/{DATASET}/`")
    st.markdown("---")
    st.markdown("### 📌 Pipeline")
    st.code(f"python run_pipeline.py --dataset {DATASET}", language="bash")
